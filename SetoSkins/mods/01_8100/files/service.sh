...
" "$MODDIR/module.prop"
  ...
  then
    sed -i "/^description=/c description=[ 充电缓慢⚠️ ️电量$capacity% 温度$temp° 电流$ChargemA"mA" ]可能是涓流，如果不是，请排查问题改最大电流目录在/data/adb/modules/SetoSkins/system/current_target 默认为22A｜temp_limit是高温降流阀值 current_limit是指定高温降流电流。
" "$MODDIR/module.prop"
    echo '0' > /sys/class/power_supply/battery/input_current_limited
    echo '1' > /sys/class/power_supply/usb/boost_current
    echo ${current_target} > /sys/class/power_supply/usb/ctm_current_max
